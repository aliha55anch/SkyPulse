...
function RightSidebar() {
  ...
  return (
    <aside className="min-h-0 w-full overflow-hidden rounded-2xl border border-white/5 bg-slate-800/70 p-3.5 shadow-[0_12px_30px_rgba(0,0,0,0.22)] max-[620px]:p-3">
      <h3 className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-slate-300">7-Day Forecast</h3>

      <div className="grid h-[calc(100%-24px)] grid-rows-7 max-[850px]:h-auto max-[850px]:grid-rows-none">
        {forecast.map((item, index) => {
          ...
          return (
            <div
              key={item.day}
              className={`flex min-h-0 items-center justify-between rounded-xl px-2 py-1 max-[850px]:py-2 ${index !== forecast.length - 1 ? 'border-b border-white/5' : ''}`}
            >
              <span className="w-14 text-xs font-bold text-slate-300">{item.day}</span>
              <Icon className="text-2xl text-amber-300" />
              <span className="flex-1 pl-2 text-xs font-bold text-slate-100">{item.label}</span>
